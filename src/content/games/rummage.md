---
title: "Rummage"
description: "Someone has added extra pieces to your jigsaw - find them and quick!"
publishedDate: "01 Feb 2026"
jamName: Queble's Jam - 2026
jamLink: https://itch.io/jam/quebles-jam-2026
itchLink: https://flatlit.itch.io/rummage
videoLink: https://www.youtube.com/watch?v=xGqqbiKUVe4
published: true
---

Click on pieces when you see them, or drag your mouse around to stir things up.

Found them? Make it harder by adding more pieces or making the colors closer.

My submission to [Queble's Gam Jam 2026](https://itch.io/jam/quebles-jam-2026), themed as _'Out of Place'_ (find those different coloured corner jigsaw pieces) and a tiny touch of _'Timing'_ (see if you can beat your time).
